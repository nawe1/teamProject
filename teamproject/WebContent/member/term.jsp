...
<style>
.container_term { width:1400px; }
.page_term { clear:both; height:900px;  }
#page1_term { background-color:#ececec;  }


.page_title { font-size:36px; padding-top:4em; text-align:center; padding-bottom:30px; }
.test{ width:1400px; margin:0 auto;   }
.form-control{width:1200px;}
.title{margin:12px;}
.btn-group_term{float:left;
...
.btn-group_term:hover {background-color: #4d574d;}
.form-check_term{margin-bottom:20px;}
</style>
</head>
<body>
<div id="header">
...
<div id="contents">
	<section class="page_term" id="page1_term">
		<c:if test="${not empty msg}">
		<div style="width:1200px; margin:0 auto;">${msg }</div>
		</c:if>
		<div class="test" style="width:1200px; margin:0 auto;">
			...
			제 1 조 (목적) 이 이용약관은 “가산동소개 (이하 "당 사이트")”에서 제공하는 인터넷 서비스(이하 '서비스')의 가입조건, 당 사이트와 이용자의 권리, 의무, 책임사항과 기타 필요한 사항을 규정함을 목적으로 합니다.
			
			제 2 조 (용어의 정의)
			1. "이용자"라 함은 당 사이트에 접속하여 이 약관에 따라 당 사이트가 제공하는 서비스를 받는 회원 및 비회원을 말합니다.
			2. “가산동소개”에서 제공하는 단체표준, 단체인증, 예고현황 정보를 말합니다.
			3. "회원"이라 함은 서비스를 이용하기 위하여 당 사이트에 개인정보를 제공하여 아이디(ID)와 비밀번호를 부여 받은 자를 말합니다.
			4. “비회원”이하 함은 회원으로 가입하지 않고 " 가산동소개"에서 제공하는 서비스를 이용하는 자를 말합니다.
			...
			제 7 장 " 가산동소개" 게시물 운영정책
			
			제 20 조 (운영정책)
			"가산동소개" 각종 게시물에 대한 운영정책은 방송통신심의위원회의 정보통신에 관한 심의규정에 기반하며 이를 위반할 경우, "가산동소개" 운영정책에 의해 관련 게시물은 예고 없이 삭제, 이동될 수 있으며, 게시자(회원)는 아이디 이용제한 등 "가산동소개" 이용에 제한을 받을 수 있습니다.
			...
			- 공인이나 특정이슈와 관련한 당사자 또는 지인, 주변인 등을 사칭하여 게시물을 작성하거나, "가산동소개" 운영자를 사칭하여 작성된 내용
			...
			① 수집항목
			가산동소개은 회원가입 및 기본적인 서비스 제공을 위한 필수정보와 추가적인 서비스 제공 등을 위해 필요한 선택정보로 구분하여 아래와 같은 개인정보를 수집하고 있습니다.
			- 필수항목 : 이름, 이메일, 전화번호, 핸드폰번호, ID, 비밀번호
			- 선택항목 : 해당사항 없음
			선택정보를 입력하지 아니하더라도 서비스 이용에 제한은 없으며, 불가피하게 수집이 필요한 경우 별도의 동의를 받는 등 필요한 조치를 이행하겠습니다.
			
			② 개인정보의 수집ㆍ이용목적
			가산동소개은 수집한 개인정보를 홈페이지 서비스 제공을 위한 회원관리 목적으로만 이용하며, 이용목적이 변경될 경우 별도의 동의를 받는 등 필요한 조치를 이행하겠습니다.
			
			③ 개인정보의 보유ㆍ이용기간
			가산동소개은 이용자의 개인정보를 회원 탈퇴 시까지만 제한적으로 이용하고 있으며, 이용자가 회원 탈퇴를 요청하는 경우 해당 이용자의 개인정보는 지체 없이 파기됩니다.
			...
			<div class="">
			  <button type="button" onclick="all_check()" class="btn-group_term">전체 약관에 동의</button>
			</div>
			<div class="">  
			  <button type="button" onclick="term_check()" class="btn-group_term">다음 단계</button>
			</div>
		</div>
		<script>
